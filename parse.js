...
function categoryNameToId(name) {
    return typeof name === 'string' && name.length ? name.replaceAll('&', 'and').toLowerCase().replaceAll(' ', '_').replaceAll('(', '').replaceAll(')', '').replaceAll('.','') : null
}
...
function machineNameToId(name) {
    return typeof name === 'string' && name.length ? name.toLowerCase().replaceAll(' ', '_').replaceAll('(', '').replaceAll(')', '').replaceAll('.','') : null
}
...
function productNameToIcon(n) {
    if (n == 'Nitrogen Tank') return 'LiquidNitrogen'
    let name = n
